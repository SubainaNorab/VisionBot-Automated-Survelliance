...
def load_model():
    if not os.path.exists(TFLITE_MODEL):
        raise FileNotFoundError(f"Model not found: {TFLITE_MODEL}")
...
def detect_biggest_face(img_bgr):
...
def preprocess_face(img_bgr, face_box, w, h, padding=PADDING):
    x, y, fw, fh = face_box
    pad = int(fw * padding)
...
def get_embedding(interpreter, input_details, output_details, face_input):
    interpreter.set_tensor(input_details[0]["index"], face_input)
    interpreter.invoke()
    emb = interpreter.get_tensor(output_details[0]["index"]).flatten()
...
def embedding_stats(vec):
    non_zero = int(np.count_nonzero(np.abs(vec) > 1e-4))
    return {
        "dims": int(vec.shape[0]),
        "non_zero": non_zero,
        "non_zero_pct": 100.0 * non_zero / float(vec.shape[0]),
        "min": float(vec.min()),
        "max": float(vec.max()),
        "l2": float(np.linalg.norm(vec)),
    }

def extract_embedding_from_image(path, interpreter, input_details, output_details, w, h, save_crop=False):
    ...
    face_box = detect_biggest_face(img_bgr)
    if face_box is None:
        raise RuntimeError(f"No face detected in: {path}")

    face_input, face_crop = preprocess_face(img_bgr, face_box, w, h)

    if save_crop:
        out = "crop_" + os.path.basename(path)
        cv2.imwrite(out, face_crop)
        print(f"💾 Saved crop: {out}")

    emb = get_embedding(interpreter, input_details, output_details, face_input)
    stats = embedding_stats(emb)

    print(f"✅ Embedding extracted: {path}")
    print(f"   Stats: dims={stats['dims']}, non_zero={stats['non_zero']} ({stats['non_zero_pct']:.1f}%), l2={stats['l2']:.4f}")
    print(f"   Range: [{stats['min']:.6f}, {stats['max']:.6f}]")

    if stats["non_zero"] < 10:
        print("⚠️ Embedding looks bad. Too many zeros.")

    return emb

def verify_pair(img1_path, img2_path, interpreter, input_details, output_details, w, h, save_crops):
    emb1 = extract_embedding_from_image(img1_path, interpreter, input_details, output_details, w, h, save_crop=save_crops)
    emb2 = extract_embedding_from_image(img2_path, interpreter, input_details, output_details, w, h, save_crop=save_crops)
    ...
    print(f"Threshold(sim): {SIM_THRESHOLD:.2f}")
    print(f"Decision: {'✅ SAME PERSON' if same else '❌ DIFFERENT'}")
    print("============================================================")
    print("")

    return same, sim, dist
...
def find_same_pairs_in_folder(folder=".", save_crops=False):
    interpreter, input_details, output_details, w, h = load_model()
    ...
    print(f"🖼️ Found {len(images)} images in: {folder}")
    print(f"🔎 Searching pairs with similarity >= {SIM_THRESHOLD:.2f}")
    print("")
    ...
    for i in range(len(images)):
        for j in range(i + 1, len(images)):
            ...
            try:
                emb1 = extract_embedding_from_image(img1, interpreter, input_details, output_details, w, h, save_crop=save_crops)
                emb2 = extract_embedding_from_image(img2, interpreter, input_details, output_details, w, h, save_crop=save_crops)
                sim = cosine_similarity(emb1, emb2)
            except Exception as e:
                print(f"❌ Skip pair due to error: {os.path.basename(img1)} vs {os.path.basename(img2)}")
                print(f"   Error: {e}")
                print("")
                continue

            if sim >= SIM_THRESHOLD:
                same_pairs.append((sim, img1, img2))
                print(f"✅ SAME: {os.path.basename(img1)}  <->  {os.path.basename(img2)}   sim={sim:.4f}")
            else:
                print(f"❌ DIFF: {os.path.basename(img1)}  <->  {os.path.basename(img2)}   sim={sim:.4f}")
    ...
    print("============================================================")
    if not same_pairs:
        print("No SAME pairs found at this threshold.")
    else:
        same_pairs.sort(reverse=True, key=lambda x: x[0])
        print(f"✅ SAME pairs found: {len(same_pairs)}")
        for sim, a, b in same_pairs:
            print(f"✅ {os.path.basename(a)} <-> {os.path.basename(b)}   sim={sim:.6f}")
    print("============================================================")

if __name__ == "__main__":
    # Option A: verify one pair
    # Uncomment and set your images
    interpreter, input_details, output_details, w, h = load_model()
    verify_pair("subaina.jpg", "test.jpg", interpreter, input_details, output_details, w, h, save_crops=True)
    verify_pair("subaina.jpg", "tt.jpeg", interpreter, input_details, output_details, w, h, save_crops=True)

    # Option B: scan all images in a folder and print which ones are same
    # find_same_pairs_in_folder(folder=".", save_crops=False)
